    	...
    	if(mDragPosition != newPosition && newPosition != AdapterView.INVALID_POSITION && mAnimationEnd){
			...
			observer.addOnPreDrawListener(new OnPreDrawListener() {
				...
				public boolean onPreDraw() {
					...
					mMobileView = mViews.get(newPosition).view;
...
//		}
	...
	private SparseArray<HeadData> mViews = new SparseArray<HeadData>();
	public void addItemView(int position, HeadData convertView) {
		if (mViews.get(position) != null) {
			mViews.remove(position);
		}
		final int i = mViews.indexOfValue(convertView);
		if (i != -1) {
			mViews.removeAt(i);
		}
...
//            }
    ...
    public static class HeadData{
    	...
    	public HeadData(long headId, View view) {
			super();
			this.headId = headId;
			this.view = view;
		}

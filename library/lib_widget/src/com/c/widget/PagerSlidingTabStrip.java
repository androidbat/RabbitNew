...
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	public void setViewTabProvider(ViewTabProvider viewTabProvider,int initPostion){
		this.viewTabProvider = viewTabProvider;
		if (pager == null) {
			for (int i = 0; i < viewTabProvider.getCount(); i++) {
				addTab(i, viewTabProvider.getPageView(i));
			}
			selectedPosition = initPostion;
			updateTabStyles();
		}
	}
	...
	public void setIndicatorRidio(int indicatorRidio){
		this.indicatorRidio = indicatorRidio;
	}
	/**
	 * 对PagerSlidingTabStrip的各项属性进行赋值。
	 */
	private void setTabsValue(PagerSlidingTabStrip tabs) {
		// 设置Tab是自动填充满屏幕的
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mLefts == null){
			mLefts = new HashMap<View,Integer>();
		}
		...
		Integer left = mLefts.get(currentTab);
		if (left == null){
			left = getTextLeft(currentTab);
			mLefts.put(currentTab,left);
		}
		...
		if (currentPositionOffset > 0f && currentPosition < tabCount - 1) {
			...
			Integer nextleft = mLefts.get(nextTab);
//			LogManager.d(TAG," nextleft "+nextleft);
			if (nextleft == null){
				nextleft = getTextLeft(nextTab);
				mLefts.put(nextTab,nextleft);
			}

			final float nextTabLeft = nextTab.getLeft()+nextleft;
			final float nextTabRight = nextTab.getRight()-nextleft;
		...
		if (indicatorRidio > 0 ){
			canvas.drawRoundRect(new RectF(lineLeft, height - indicatorHeight-indicatorPaddingBottom, lineRight, height-indicatorPaddingBottom),indicatorRidio,indicatorRidio,rectPaint);
		}else{
	...
	private int getTextLeft(View tab){
		int left = 0;
		if(isWrapText){
//			try {
//				TextView tv = (TextView) tab.findViewById(R.id.tab_textview);
//				if(tv == null){
//					tv = (TextView) tab;
//				}
//				float textWidth =  tv.getPaint().measureText((String) tv.getText());
//				left = (int) ((tab.getWidth()-textWidth*1.1)/2)-5;
//			} catch (Exception e) {
//				e.printStackTrace();
//			}
		}
	...
	private void select(int selection){
		...
		int newScrollX = tabsContainer.getChildAt(selection).getLeft();
